...
const EventRequestModalOffline = ({ isOpen, onClose, selectedDate }) => {
  const { user } = useAuth()
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      // Validate file size (max 1MB for offline storage)
      if (file.size > 1024 * 1024) {
        setFileError('Die Datei ist zu groß. Maximale Größe: 1MB für Offline-Speicherung.')
        return
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Try to insert with a very short timeout
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Database timeout - saving locally instead')), 3000)
      )

      try {
        const { data, error: requestError } = await Promise.race([
          insertPromise,
          timeoutPromise
        ])
        ...
        console.log('✅ Event request created in database:', data)
        setSuccess(true)
        ...
        console.log('✅ Event request saved locally:', localData)
        setSuccess(true)
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Event anfragen (Offline-fähig)</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {success && (
            <div className="bg-green-50 border border-green-200 rounded-md p-4">
              <p className="text-sm text-green-800">✅ Event-Anfrage erfolgreich erstellt!</p>
            </div>
          )}
          ...
          <div>
            ...
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
              <div className="space-y-1 text-center">
                ...
                <p className="text-xs text-gray-500">PDF bis zu 1MB</p>
                {fileError && (
...
export default EventRequestModalOffline
